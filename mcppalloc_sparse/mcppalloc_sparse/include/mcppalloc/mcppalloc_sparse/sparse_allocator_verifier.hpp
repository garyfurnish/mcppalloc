...
namespace mcppalloc::sparse::details
{
  ...
  void sparse_allocator_verifier_t::verify_block_new(Allocator &allocator, Block &block)
  {
    ...
    for (auto &&it : allocator.m_blocks) {
      // it is a fatal error to try to double add and something is inconsistent.
      // Terminate before memory corruption spreads.
      if (it.m_block == &block) {
        ::std::abort();
      }
      // it is a fatal error to try to double add and something is inconsistent.
      ...
      if (it.m_begin == block.begin()) {
        ::std::cerr << " Attempt to double register block. "
                       "77dbea01-7e0f-49da-81f1-9ad7f4616eea\n";
  ...
  void sparse_allocator_verifier_t::verify_blocks_sorted(Allocator &allocator)
  {
    ...
    if (!allocator.m_blocks.empty() &&
      ...
      ::std::cerr << "sparse allocator blocks out of order. "
                     "6df4aa74-b7b4-453d-a044-41f6d5e38b9b"
                  << ::std::endl;
      ::std::abort();
    }
  }
