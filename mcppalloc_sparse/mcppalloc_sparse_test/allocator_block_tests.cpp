...
void allocator_block_tests()
{
  describe("Block", []() {
    ...
    void *alloc4 = nullptr;
    it("alloc1", [&]() {
      alloc1 = get_allocated_memory(block.allocate(15));
    ...
    it("alloc2", [&]() {
      alloc2 = get_allocated_memory(block.allocate(15));
    ...
    it("alloc3", [&]() {
      alloc3 = get_allocated_memory(block.allocate(15));
    ...
    it("alloc4", [&]() {
      alloc4 = get_allocated_memory(block.allocate(15));
    ...
    it("realloc 1&2 #3", [&]() {
      ...
      block.destroy(alloc1);
      AssertThat(block.m_free_list, HasLength(1));
      alloc1 = get_allocated_memory(block.allocate(48));
      AssertThat(alloc1, Equals(old_alloc1));
      AssertThat(object_state_type::from_object_start(alloc1)->next_valid(), IsTrue());
      AssertThat(object_state_type::from_object_start(alloc1)->not_available(), IsTrue());
      AssertThat(block.m_free_list, HasLength(0));
    ...
    it("Fail realloc 1&2 #2", [&]() {
      ...
      block.destroy(alloc1);
      AssertThat(block.m_free_list, HasLength(1));
      AssertThat(get_allocated_memory(block.allocate(17 + 2 * aligned_header_size)), Equals(static_cast<void *>(nullptr)));
      alloc1 = get_allocated_memory(block.allocate(15));
      AssertThat(alloc1, Equals(old_alloc1));
      AssertThat(object_state_type::from_object_start(alloc1)->next_valid(), IsTrue());
      AssertThat(object_state_type::from_object_start(alloc1)->not_available(), IsTrue());
    ...
    it("fail massive alloc", []() {
      ...
      AssertThat(get_allocated_memory(block2.allocate(985)), Equals(static_cast<void *>(nullptr)));
      free(memory2);
    });
    it("empty", []() {
      void *memory2 = malloc(1000);
      ...
      auto m = get_allocated_memory(block2.allocate(500));
